 ...
 * Written by yurunsun@gmail.com, based on Nate Robbins's original
 * glmDimensions function, then re-wrote glmDimensions to use this.
 *
 * model   - initialized GLMmodel structure
 * bmin	   - array of 3 GLfloats (GLfloat bmin[3])
 * bmax	   - array of 3 GLfloats (GLfloat bmax[3])
 */
GLvoid
glmBoundingBox(GLMmodel* model, GLfloat* bmin, GLfloat* bmax)
{
    GLuint i;
    GLfloat maxx, minx, maxy, miny, maxz, minz;
...
glmDimensions(GLMmodel* model, GLfloat* dimensions)
{
	...
	GLfloat bmin[3];
	GLfloat bmax[3];
	glmBoundingBox(model, bmin, bmax);

	// Computation changed by yurunsun@gmail.com
	dimensions[0] = glmAbs(bmax[0] - bmin[0]);
	dimensions[1] = glmAbs(bmax[1] - bmin[1]);
	dimensions[2] = glmAbs(bmax[2] - bmin[2]);
}
...
glmVertexNormals(GLMmodel* model, GLfloat angle)
{
  ...
  members.reserve(model->numvertices + 1);
  ...
  for (i = 0; i < model->numtriangles; i++) {
    node = new GLMnode;//(GLMnode*)malloc(sizeof(GLMnode));
    node->index = i;
  ...
  //free(members);
  //delete [] members;
  for (std::vector<GLMnode*>::iterator it = members.begin(); it != members.end(); ++it){
	  delete (*it);
	  *it = NULL;
  }
  members.clear();
